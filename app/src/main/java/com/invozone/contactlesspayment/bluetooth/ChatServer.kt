 ...
 */
package com.invozone.contactlesspayment.bluetooth
...
import com.invozone.contactlesspayment.bluetooth.Message.RemoteMessage
import com.invozone.contactlesspayment.chat.DeviceConnectionState
...
object ChatServer {
    ...
    fun determineRole() {
        ...
        if (service != null) {
            ...
            Log.d(TAG, MESSAGE_UUID.toString())
            // Check if the service contains the MESSAGE_UUID characteristic
            val messageCharacteristic = service.getCharacteristic(MESSAGE_UUID)

            if (messageCharacteristic != null) {
                // If the MESSAGE_UUID characteristic is found, it's acting as a client
                Log.d(TAG, "Device is acting as a client")
            } else {
                // If the MESSAGE_UUID characteristic is not found, it's acting as a server
                Log.d(TAG, "Device is acting as a server")
            }
        } else {
            // If the service is not found, it's not clearly identified as a client or server
            Log.d(TAG, "Role cannot be determined (neither server nor client)")
        }
    }
...
//        _deviceConnection.value=DeviceConnectionState.Disconnected
        Log.d(TAG, "Attempting to cancel connection for $currentDevice")
        try {
            gattServer?.cancelConnection(currentDevice)
            _deviceConnection.value=DeviceConnectionState.Disconnected
            _connectionRequest.value = null
            isServer=false
            Log.d(TAG, "Connection canceled successfully")
        } catch (e: Exception) {
            Log.e(TAG, "Error while canceling connection: ${e.message}")
    ...
    fun sendMessage(message: String, amount: Int): Boolean {
        ...
        messageCharacteristic?.let { characteristic ->
            ...
            gatt?.let {
                ...
                if (success) {
                    _messages.value = Message.LocalMessage(message,amount)
    ...
    private class GattServerCallback : BluetoothGattServerCallback() {
        ...
        override fun onCharacteristicWriteRequest(
            ...
            if (characteristic.uuid == MESSAGE_UUID) {
                ...
                message?.let {
                    _messages.postValue(RemoteMessage(it,2000))
                }
            }
        }
    }
